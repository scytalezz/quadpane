...
빌드 결과인 `dist-portable/quadpane-0.4.0-portable-x64.exe`를 쓰기 가능한 폴더에 두고 실행합니다. 설치, 관리자 권한, 별도 Node.js가 필요하지 않습니다. Electron 런타임을 포함합니다.
...
다른 앱으로 보낼 때는 `Alt`를 누른 채 드래그를 시작합니다. 외부 앱으로 실제 끌어 놓기를 완료하는 동작은 아직 수동 검증하지 않았습니다. 복사·잘라내기·붙여넣기는 Quadpane 내부 대기 목록을 사용하며 Windows 클립보드와 공유하지 않습니다.
...
파일 두 번 클릭과 `Enter`는 Windows 기본 앱을 호출합니다. **파일 정보** 또는 `Alt+Enter`로 메타데이터를 확인하고, **탐색기에서 보기**로 해당 항목을 Windows 탐색기에 표시합니다. 오른쪽 클릭 메뉴는 Quadpane이 제공하는 기본 메뉴이며 Windows 셸 확장 메뉴를 포함하지 않습니다.
